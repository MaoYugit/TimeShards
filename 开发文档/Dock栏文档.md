...
为了避免“轻点一下就卡在放大状态”，移动端 Dock 的规则是：

- **轻点（tap）**：不触发 hover 放大（只执行点击行为）
- **拖动（drag）**：当手指移动超过约 `8px` 后，开始触发 hover 放大，并跟随手指移动
- **抬起/取消触摸**：立刻恢复原状（复位内部坐标）

这样手机也能体验类似 hover 的放大联动，但不会因为一次点击而停留在放大状态。

---

### 点击动效

Dock 按钮点击 **不做弹跳动画**，点击只负责导航/切换主题。
